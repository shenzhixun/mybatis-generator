...
public class MyJavaMapperMethodGenerator extends AbstractJavaMapperMethodGenerator {
    ...
    private void addInterfaceInsertBatch(Interface interfaze) {
        ...
        FullyQualifiedJavaType exception = new FullyQualifiedJavaType(CoBusinessException.class.getName());
        method.addException(exception);
        importedTypes.add(exception);
        ...
        // 设置返回类型是List
        FullyQualifiedJavaType listParameter = FullyQualifiedJavaType.getNewListInstance();
        FullyQualifiedJavaType listType;
        // 设置List的类型是实体类的对象
        listType = new FullyQualifiedJavaType(introspectedTable.getBaseRecordType());
        importedTypes.add(listType);
        // 返回类型对象设置为List
        listParameter.addTypeArgument(listType);
        // 设置方法名称为我们在IntrospectedTable类中初始化的 “insertBatch”
        method.setName(config.METHOD_INSERT_BATCH);
        // 为方法添加参数，变量名称record
        method.addParameter(new Parameter(listParameter, "list")); //$NON-NLS-1$
